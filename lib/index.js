  ...
  const enable = async () => {
    ...
    try {
      ...
      const child = cspawn(process.execPath, [DAEMON], {
        detached: true,
        stdio: 'ignore',
        env: { ...process.env, DSH_PROXY_USER: 'dsh', DSH_PROXY_PASS: password, DSH_PROXY_PORT: PROXY_PORT, DSH_CAFFEINATE: cfg.caffeinate ? '1' : '0' }
      })
      child.unref()
      let s = null
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST' && pathname === '/iptunnel/settings') {
          let v = false
          try { v = new URL(req.url, 'http://localhost').searchParams.get('caffeinate') === '1' } catch (e) {}
          cfg.caffeinate = v
          saveSettings()

...
export async function apply(ctx) {
  ...
  // loopback client (127.0.0.1 / localhost) or to a public request that came
  // through the pool itself (Host = *.trycloudflare.com, already behind the
  // auth proxy). A LAN-access Host (e.g. 192.168.x.x) gets 403.
  const hostOf = (req) => {
  ...
  const route = {
    ...
    handler: async (req, res) => {
      ...
      const h = hostOf(req)
      if (h !== '127.0.0.1' && h !== 'localhost' && h !== '::1' && !h.endsWith('.trycloudflare.com')) return json(res, 403, { error: 'forbidden' })
      try {

...
import { rmSync, readFileSync, writeFileSync, statSync } from 'node:fs'
...
export async function apply(ctx) {
  ...
  // resolve the cloudflared binary: env override, then the usual brew/system
  // homes (the host's PATH may be restricted, e.g. launched from a GUI or a
  // launchd agent with a minimal PATH), then the PATH name itself.
  const resolveCloudflared = () => {
    if (process.env.DSH_CLOUDFLARED) return process.env.DSH_CLOUDFLARED
    const candidates = ['/opt/homebrew/bin/cloudflared', '/usr/local/bin/cloudflared', '/usr/bin/cloudflared']
  ...
  const enable = async () => {
    ...
    try {
      ...
      const child = cspawn(process.execPath, [DAEMON], {
        detached: true,
        stdio: 'ignore',
        env: { ...process.env, DSH_PROXY_USER: 'dsh', DSH_PROXY_PASS: password, DSH_PROXY_PORT: PROXY_PORT, DSH_CAFFEINATE: cfg.caffeinate ? '1' : '0', DSH_MAX_GENS: String(cfg.maxGens), DSH_ROTATE_MS: String(cfg.rotateH * 3600000), DSH_CLOUDFLARED: resolveCloudflared() }
      })
      child.unref()
      let s = null
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST' && pathname === '/iptunnel/refresh') {
          ...
          // once every prereq is satisfied a stale enable-failure no longer
          // applies: the yellow warning already vanished, drop the red line too
          const prOk = Object.keys(state.prereqs).every((k) => state.prereqs[k] && state.prereqs[k].ok === true)
          if (prOk && state.error !== null) state.error = null

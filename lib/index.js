...
export async function apply(ctx) {
  ...
  const state = { phase: 'disabled', enabled: false, url: null, username: 'dsh', password: null, startedAt: null, error: null, qr: null }
  ...
  const enable = async () => {
    ...
    try {
      ...
      const s = await waitFor(() => {
        const st = readStore()
        return st !== null && typeof st.url === 'string' && st.url !== '' ? st : null
      }, 180, 500, 'daemon store')
      state.url = s.url
      state.username = s.username || 'dsh'

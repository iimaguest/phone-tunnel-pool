...
export async function apply(ctx) {
  ...
  const checkPrereqs = async () => {
    ...
    p.cloudflared = { ok: await okCmd([cf, '--version']), hint: cf + ' not found on PATH — brew install cloudflared (or set DSH_CLOUDFLARED)' }
    p.qr = { ok: await okCmd([PYTHON, '-c', 'import qrcode']), hint: 'python3 qrcode package missing — pip install qrcode (URL + login still usable)' }
    return p
  }
  ...
  const enable = async () => {
    ...
    try {
      ...
      const child = cspawn(process.execPath, [DAEMON], {
        detached: true,
        stdio: 'ignore',
        env: { ...process.env, DSH_PROXY_USER: 'dsh', DSH_PROXY_PASS: password, DSH_PROXY_PORT: PROXY_PORT }
      })
      child.unref()
      let s = null

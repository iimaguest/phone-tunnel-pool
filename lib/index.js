...
export async function apply(ctx) {
  ...
  const cfg = { caffeinate: false, autoEnable: false, maxGens: 4, autoDisableH: 0 }
  try {
    ...
    if (Number.isFinite(s.maxGens)) cfg.maxGens = Math.max(1, Math.min(6, Math.floor(s.maxGens)))
    if (Number.isFinite(s.autoDisableH)) cfg.autoDisableH = Math.max(0, Math.min(48, Math.floor(s.autoDisableH)))
  } catch (e) {}
  ...
  const enable = async () => {
    ...
    try {
      ...
      const child = cspawn(process.execPath, [DAEMON], {
        detached: true,
        stdio: 'ignore',
        env: { ...process.env, DSH_PROXY_USER: 'dsh', DSH_PROXY_PASS: password, DSH_PROXY_PORT: PROXY_PORT, DSH_CAFFEINATE: cfg.caffeinate ? '1' : '0', DSH_MAX_GENS: String(cfg.maxGens) }
      })
      child.unref()
      let s = null
  ...
  const snapshot = () => {
    ...
    return {
      ...
      rotateH: Math.round((Number(process.env.DSH_ROTATE_MS || 12 * 3600000) / 3600000)),
      ...(pool !== null ? {
        pool: {
  ...
  ctx.effect(() => {
    ...
    const id = setInterval(async () => {
      ...
      // auto-disable: leave the tunnel open too long? close it for you.
      if (cfg.autoDisableH > 0 && state.startedAt && Date.now() - state.startedAt > cfg.autoDisableH * 3600000) {
      ...
      try {
        ...
        if (req.method === 'POST' && pathname === '/iptunnel/settings') {
          try {
            ...
            if (q.get('mg')) cfg.maxGens = Math.max(1, Math.min(6, Math.floor(Number(q.get('mg')) || 4)))
            if (q.get('ad')) cfg.autoDisableH = Math.max(0, Math.min(48, Math.floor(Number(q.get('ad')) || 0)))
          } catch (e) {}
          saveSettings()

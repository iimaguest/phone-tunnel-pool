...
// Re-run on maintenance too: a long-lived daemon should keep the temp copies
// current after an update (the proxy reads its watchdog copy at boot).
const RUNTIME_FILES = [['cf-auth-proxy.mjs', PROXY], ['iptunnel-sw.js', TMP + '/iptunnel-sw.js'], ['iptunnel-watchdog.js', TMP + '/iptunnel-watchdog.js']]
const refreshRuntimeFiles = () => {
  for (const [name, target] of RUNTIME_FILES) {

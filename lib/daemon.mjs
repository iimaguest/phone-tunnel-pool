...
import { copyFileSync, writeFileSync, chmodSync, rmSync, appendFileSync, renameSync } from 'node:fs'
...
const spawnTunnel = async (genObj, id, replace) => {
  ...
  try {
    proc = spawn(CLOUDFLARED, [
      'tunnel', '--url', 'http://127.0.0.1:' + PROXY_PORT,
      '--no-autoupdate', '--no-prechecks',
      '--metrics', '127.0.0.1:' + port,
      '--loglevel', 'info', '--transport-loglevel', 'error',
      '--grace-period', '5s', '--edge-ip-version', '4',
      '--post-quantum', '--management-diagnostics=false',
    ], { stdio: ['ignore', 'pipe', 'pipe'] })
  } catch (e) { log('gen' + genObj.gen + '/' + id + ' spawn FAILED: ' + e.message); return t }
  t.proc = proc
  t.proc.on('error', (e) => {
    log(e.code === 'ENOENT'
      ? 'cloudflared binary not found on PATH — install it (brew install cloudflared) or set DSH_CLOUDFLARED'
      : 'cloudflared spawn error: ' + e.message)
    stop(1, e.code === 'ENOENT' ? 'cloudflared-not-found' : 'cloudflared-spawn-error')
  })
  t.pid = proc.pid
  let buf = ''
  const onData = (d) => {
...
// ALWAYS copy: /tmp may hold a stale v1 copy, and source is the source of truth.
...
// a fresh daemon session owns /tmp state; clear any stale file first
try { rmSync(STORE) } catch (e) {}
// version receipt
try {
...
// keep the Mac awake while the daemon exists
const spawnAwake = () => {
  try {
    awake = spawn('/usr/bin/caffeinate', ['-dims'], { stdio: 'ignore' })

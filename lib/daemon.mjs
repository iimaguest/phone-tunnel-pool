...
const tunnelArgs = (metricsPort) => {
  ...
  if (cfGte(2024, 6)) a.push('--post-quantum')
...
const spawnTunnel = async (genObj, id, replace) => {
  ...
  let proc
  try {
    proc = spawn(CLOUDFLARED, tunnelArgs(port), { stdio: ['ignore', 'pipe', 'pipe'] })
  } catch (e) {
...
// keep the machine awake while the daemon exists (macOS-only utility;
// on Linux/Windows there is no caffeinate — skip instead of crashing)
const spawnAwake = () => {

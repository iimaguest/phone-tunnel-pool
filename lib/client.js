      ...
      if (open) {
        ...
        panel = React.createElement('div', { style: panelStyle, ref: panelRef, className: 'pt-panel' },
          ...
          React.createElement('div', { style: { marginTop: 8, display: 'flex', alignItems: 'center', gap: 10 } },
            ...
            React.createElement('div', { style: { fontSize: 12, color: P.dim2 } }, 'Status: ' + (state === null ? 'loading…' : state.phase) + (busy ? ' (working…)' : ''))),
          ...
          state !== null && state.error ? React.createElement('div', { style: { color: P.err, marginTop: 4 } }, 'Error: ' + state.error) : null,
          err !== null ? React.createElement('div', { style: { color: P.err, marginTop: 4 } }, 'Error: ' + err) : null,
          ...
          enabled && state !== null && state.url ? React.createElement('div', null,
            ...
            React.createElement('div', { style: { fontSize: 11, color: P.dim, marginTop: 4 } }, 'Phone camera → scan → Safari opens the site and uses the login embedded in the QR. Anyone who photographs the card can open the tunnel, so disable it when you are done.'),

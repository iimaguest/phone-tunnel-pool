    ...
    var TunnelWidget = function TunnelWidget() {
      ...
      if (open) {
        ...
        panel = React.createElement('div', { style: panelStyle, ref: panelRef },
          ...
          state !== null && state.os === 'darwin' ? React.createElement('label', { style: { display: 'flex', alignItems: 'center', gap: 6, marginTop: 8, fontSize: 12, color: 'rgba(200,200,200,0.9)', cursor: 'pointer' } },
            React.createElement('input', { type: 'checkbox', checked: !!(state.settings && state.settings.caffeinate), style: { cursor: 'pointer' },
              onChange: function (e) {
                fetch('/iptunnel/settings?caffeinate=' + (e.target.checked ? '1' : '0'), { method: 'POST', headers: { accept: 'application/json' } })
                  .then(function (r) { return r.json() })
                  .then(function (s) { setState(s) }).catch(function (err) { setErr(String(err && err.message ? err.message : err)) })
              } }),
            React.createElement('span', null, 'Keep machine awake while enabled (caffeinate)'),
            React.createElement('span', { style: { fontSize: 11, color: 'rgba(170,170,170,0.7)' } }, '— applies on next Enable')) : null,

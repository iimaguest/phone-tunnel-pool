...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var TunnelWidget = function TunnelWidget() {
      ...
      var pillStyle = { position: 'fixed', right: 16, top: topPos === null ? undefined : topPos, bottom: topPos === null ? 16 : undefined, zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34, background: 'rgba(24,26,30,0.92)', border: '1px solid rgba(128,128,128,0.55)', borderRadius: 9, boxShadow: '0 2px 10px rgba(0,0,0,0.38)', padding: 0, cursor: 'pointer', fontFamily: 'system-ui, sans-serif', fontSize: 20, touchAction: 'none', userSelect: 'none' }
      // subtle state from the icon itself: green glow when the tunnel is live,
      // neutral (still clearly visible) when disabled
      var iconStyle = enabled
        ? { textShadow: '0 0 7px rgba(46,160,67,0.65)' }
        : { textShadow: '0 1px 3px rgba(0,0,0,0.35)' }
      var panelStyle = { position: 'fixed', right: 16, bottom: 64, zIndex: 9000, width: 340, maxWidth: 'calc(100vw - 32px)', maxHeight: 'min(72vh, 560px)', overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: 12, borderRadius: 12, border: '1px solid rgba(128,128,128,0.4)', background: 'rgba(24,26,30,0.96)', color: '#e8e8e8', fontFamily: 'system-ui, sans-serif', fontSize: 13, lineHeight: 1.5, boxShadow: '0 8px 28px rgba(0,0,0,0.4)' }
        ...
        panel = React.createElement('div', { style: panelStyle, ref: panelRef },
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 } },
            ...
            React.createElement('button', {
              ...
              title: 'Refresh status (re-checks installed prerequisites)',
              onClick: function () {
          ...
          state !== null && state.settings && more ? (function () {
            ...
            var slider = function (key, label, val, min, max, dflt, fmt) {
              return React.createElement('div', { key: key, style: L({ display: 'grid', gap: 2 }) },
                ...
                React.createElement('div', { style: L({ display: 'flex', alignItems: 'center', gap: 6 }) },
                  React.createElement('input', { type: 'range', min: min, max: max, step: 1, value: val, style: L({ flex: 1, cursor: 'pointer', accentColor: '#d4a72c' }),
                    onChange: function (e) { var n = {}; n[key] = Number(e.target.value); save(Object.assign({}, cur, n)) } }),
                  React.createElement('span', { style: L({ fontSize: 11, color: 'rgba(170,170,170,0.9)', minWidth: 44, textAlign: 'right' }) }, fmt(val)),
                  React.createElement('button', {
                    style: L({ background: 'transparent', border: '1px solid rgba(170,170,170,0.4)', color: 'rgba(220,220,220,0.9)', borderRadius: 4, fontSize: 11, padding: '1px 6px', cursor: 'pointer', opacity: val === dflt ? 0.35 : 1 }),
                    'aria-label': 'Reset ' + label + ' to default',
                    title: 'Reset to default (' + fmt(dflt) + ')',
            ...
            rows.push(slider('maxGens', 'Max generations (×2 tunnels, next Enable)', stg.maxGens, 2, 8, 4, function (v) { return String(v) }))
            rows.push(slider('rotateH', 'Rotate hostname every:', stg.rotateH, 4, 168, 12, fmtH))
            rows.push(React.createElement('div', { key: 'info', style: L({ fontSize: 11, color: 'rgba(170,170,170,0.75)', marginTop: 4 }) },
              'Target 127.0.0.1:' + (state.targetPort || '3080') + ' — env overrides: DSH_TARGET_PORT, DSH_CLOUDFLARED, DSH_IDLE_MS'))
            return React.createElement('div', { style: { marginTop: 8, fontSize: 12, color: 'rgba(200,200,200,0.9)', display: 'grid', gap: 6 } }, rows)
          })() : null,

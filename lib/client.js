...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var TunnelWidget = function TunnelWidget(props) {
      ...
      // palette is exactly the previously finalized look. Computed on every
      // render: the dsh theme plugin sets inline color-scheme on <html>, and
      // a MutationObserver below re-renders when Settings -> Appearance flips.
      var currentAppTheme = function () {
        try {
      ...
      var appThemeSt = React.useState(currentAppTheme)
      var appTheme = appThemeSt[0]
      var setAppTheme = appThemeSt[1]
      // re-render when Settings -> Appearance changes the theme: the dsh
      // theme plugin sets inline color-scheme (+ maybe a class) on <html>
      React.useEffect(function () {
        try {
          var mo = new MutationObserver(function () { setAppTheme(currentAppTheme()) })
      ...
      if (open) {
        ...
        if (enabled && state !== null && state.pool && state.pool.gens && state.pool.gens.length > 0) {
          ...
          state.pool.gens.forEach(function (g) {
            g.tunnels.forEach(function (tn) {
              var u = usage[tn.host || '']
              rows.push(React.createElement('div', { key: g.gen + '/' + tn.id, style: { padding: '4px 0', borderTop: '1px solid ' + P.rowDiv, fontSize: 12, fontFamily: 'monospace' } },
                React.createElement('span', { style: { color: g.status === 'current' ? P.cur : P.muted } }, 'gen' + g.gen + '/' + tn.id),
                ' ' + (tn.host || '?').replace(/\.trycloudflare\.com$/, '…'),
                g.status === 'current' ? ' ★' : (g.status === 'retired' ? ' ⏹' : ' ▶'),
                React.createElement('span', { style: { color: tn.healthy ? P.ok : P.dead } }, ' ●'),
